...
export const ProductCheckoutPage = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            const baseUrl: string = `http://localhost:8080/books/${productId}`;
            ...
            const loadedProduct: ProductModel = {
                ...
                product_description: responseJson.product_description,
                quantities: responseJson.quantities,
                quantity_available: responseJson.quantity_available,
    ...
    useEffect(() => {
        const fetchProductReviews = async () => {
            const reviewUrl: string = `http://localhost:8080/reviews/search/findByProductId?productId=${productId}`;
            ...
            for (const key in responseData) {
                loadedReviews.push({
                    review_id: responseData[key].review_id,
    ...
    useEffect(() => {
        const fetchUserReviewProduct = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://localhost:8080/reviews/secure/user/product/?productId=${productId}`;
    ...
    useEffect(() => {
        const fetchUserCurrentLoansCount = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://localhost:8080/products/secure/currentloans/count`;
                const requestOptions = {
                    method: 'GET',
                    headers: { 
                        Authorization: `Bearer ${authState.accessToken?.accessToken}`,
    ...
    useEffect(() => {
        const fetchUserCheckedOutProduct = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://localhost:8080/products/secure/ischeckedout/byuser/?productId=${productId}`;
                const requestOptions = {
                    method: 'GET',
                    headers: {
                        Authorization: `Bearer ${authState.accessToken?.accessToken}`,
    ...
    async function checkoutBook() {
        const url = `http://localhost:8080/products/secure/checkout/?productId=${product?.product_id}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    async function submitReview(starInput: number, reviewDescription: string) {
        ...
        const url = `http://localhost:8080/reviews/secure`;
        const requestOptions = {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    return (
        <div>
            <div className='container d-none d-lg-block'>
                <div className='row mt-5'>
                    <div className='col-sm-2 col-md-2'>
                        {product?.image ?
                            <img src={product?.image} width='226' height='349' alt='Book' />
                            :
                ...
                <hr />
                <LatestReviews reviews={reviews} productId={product?.product_id} mobile={false} />
            </div>
            <div className='container d-lg-none mt-5'>
                ...
                <CheckoutAndReviewBox product={product} mobile={true} currentLoansCount={currentLoansCount} 
                    isAuthenticated={authState?.isAuthenticated} isCheckedOut={isCheckedOut} 
                    checkoutProduct={checkoutProduct} isReviewLeft={isReviewLeft} submitReview={submitReview}/>
                <hr />
                <LatestReviews reviews={reviews} productId={product?.product_id} mobile={true} />
            </div>
        </div>
    );
}